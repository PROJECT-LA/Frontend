...
import { delay, InterpreteMensajes, print, titleCase, siteName } from "@/utils";
...
export default function UsersClientPage({ permissions }: UsersClientPageProps) {
  // Roles de usuario
      ...
      <Grid key={`${usuarioData.id}-${indexUsuario}-acciones`}>
        ...
        {permissions.delete && (
          <IconoTooltip
            ...
            color={"secondary"}
            accion={() => {
              print(`Eliminaremos`, usuarioData);
  ...
  const cambiarEstadoUsuarioPeticion = async (usuario: UsuarioRolCRUDType) => {
    try {
      ...
      print(`respuesta inactivar usuario: ${respuesta}`);
